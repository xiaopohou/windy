...
using Windy.WebMVC.Utility.MvcExtension;
...
namespace Windy.WebMVC.Areas.weixin.Controllers
{
    public class WeiXinCheckController : Controller
    {
        ...
        {
            LogManager.Instance.WriteLog("windy在请求了");
            if (!CheckSignature.Check(postModel.Signature, postModel.Timestamp, postModel.Nonce, Token))
            {
                return Content("参数错误！");
            }

            postModel.Token = ServerParam.Instance.WeiXinAppInfo.Token;
            postModel.EncodingAESKey = ServerParam.Instance.WeiXinAppInfo.EncodingAESKey;//根据自己后台的设置保持一致
            postModel.AppId = ServerParam.Instance.WeiXinAppInfo.AppID;//根据自己后台的设置保持一致
